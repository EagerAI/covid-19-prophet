...
search_grid_prophet <- data.frame(
                                  ...
                                  seasonality.prior.scale= runif(5,0, 2), 
                                  holidays.prior.scale= runif(5,0, 2) , changepoint.prior.scale= runif(5,0, 2) , 
                                  mcmc.samples=runif(5,0,50) , interval.width=runif(5,0,1) , 
...
data = forecast %>% select(ds,yhat) %>% left_join(df) %>% mutate(ds=as.Date(ds)) #%>%filter(ds>='2020-08-01')
